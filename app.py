...
def sim():
    ...
    if len(baustellen.keys()) > 1:
        ValueError("More than one baustelle detected. Please ensure there is one baustelle per map.")
    simErgebnis = simulate(laster, baustellen["A"]["größe"], map_config["distances"]["Aw"]["duration"])
    ...
    for k in map_config["routes"].keys():
       if k[0] == "w":
           m.add_child(folium.PolyLine(map_config["routes"][k]["polyline"], color="green"))
